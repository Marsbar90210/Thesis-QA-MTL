...
def compile_model(inputs, repeat):
    if (repeat == 1):
        print("This model does not support single tasks")
        return None
    ...
    with sess.as_default():
        ...
        for i in range(repeat):
            stories.append(tf.placeholder(tf.int32, [None, max_sent, sen_maxlen]))
            queries.append(tf.placeholder(tf.int32, [None, sen_maxlen]))
            answers.append(tf.placeholder(tf.int32, [None, vocab_size]))
        ...
        def fitf(trainX, trainY, t):
            feed_dict = {s: d for s, d in zip(stories, trainX[0::2])}
            feed_dict.update({q: d for q, d in zip(queries, trainX[1::2])})
            feed_dict.update({a: d for a, d in zip(answers, trainY)})
            feed_dict.update({lr: LRF(t)})
