...
print("Re-implementation of MemN2N model in tf")
...
def add_gradient_noise(t, stddev=1e-3, name=None):
    t = tf.convert_to_tensor(t, name="t")
    gn = tf.random_normal(tf.shape(t), stddev=stddev, seed=_seed)
...
MAX_GRAD_NORM = 40
NONLIN = None
#INIT = tf.random_normal_initializer(stddev=0.1, seed=_seed)
...
def compile_model(inputs, repeat):
    ...
    vocab_size2, max_sent2, sen_maxlen2, query_maxlen2 = inputs[1]
    ...
    with sess.as_default():
        ...
        stories = [tf.placeholder(tf.int32, [None, max_sent, sen_maxlen]),
                   tf.placeholder(tf.int32, [None, max_sent2, sen_maxlen2])]
        queries = [tf.placeholder(tf.int32, [None, sen_maxlen]),
                   tf.placeholder(tf.int32, [None, sen_maxlen2])]
        answers = [tf.placeholder(tf.int32, [None, vocab_size]),
                   tf.placeholder(tf.int32, [None, vocab_size2])]
        ...
        A_1 = tf.Variable(A, name="A")

        C = []

        for hopn in range(HOPS):
            with tf.variable_scope('hop_{}'.format(hopn)):
                C.append(tf.Variable(VC, name="C"))
        ...
        q_emb = [tf.nn.embedding_lookup(A_1, query) for query in queries]
        u_0 = [tf.reduce_sum(x * encoding, 1) for x in q_emb]
        u = [u_0]

        for hopn in range(HOPS):
            if hopn == 0:
                m_emb_A = [tf.nn.embedding_lookup(A_1, story) for story in stories]
                m_A = [tf.reduce_sum(x * encoding, 2) for x in m_emb_A]

            else:
                with tf.variable_scope('hop_{}'.format(hopn - 1)):
                    m_emb_A = [tf.nn.embedding_lookup(C[hopn - 1], story) for story in stories]
                    m_A = [tf.reduce_sum(x * encoding, 2) for x in m_emb_A]

            # hack to get around no reduce_dot
            u_temp = [tf.transpose(tf.expand_dims(x, -1), [0, 2, 1]) for x in u[-1]]
            dotted = [tf.reduce_sum(t1 * t2, 2) for t1, t2 in zip(m_A, u_temp)]

            # Calculate probabilities
            probs = [tf.nn.softmax(d) for d in dotted]

            probs_temp = [tf.transpose(tf.expand_dims(x, -1), [0, 2, 1]) for x in probs]
            with tf.variable_scope('hop_{}'.format(hopn)):
                m_emb_C = [tf.nn.embedding_lookup(C[hopn], story) for story in stories]
            m_C = [tf.reduce_sum(x * encoding, 2) for x in m_emb_C]

            c_temp = [tf.transpose(x, [0, 2, 1]) for x in m_C]
            o_k = [tf.reduce_sum(t1 * t2, 2) for t1, t2 in zip(c_temp, probs_temp)]

            # Dont use projection layer for adj weight sharing
            # u_k = tf.matmul(u[-1], H) + o_k

            u_k = [t1 + t2 for t1, t2 in zip(u[-1],o_k)]

            # nonlinearity - need extension to MTL
            #if NONLIN:
            #    u_k = NONLIN(u_k)
        ...
        with tf.variable_scope('hop_{}'.format(HOPS)):
            if WS:
                logits = [tf.matmul(x, tf.transpose(C[-1], [1,0])) for x in u_k]
            else:
                logits = [tf.matmul(x, W) for x in u_k]
        
        # Loss functions and training
        cross_entropy = [tf.nn.softmax_cross_entropy_with_logits(logits=t1, labels=tf.cast(t2, tf.float32))
            for t1, t2 in zip(logits, answers)]
        cross_entropy_sum = [tf.reduce_sum(ce) for ce in cross_entropy]

        # loss op
        loss_op = cross_entropy_sum

        # gradient pipeline
        grads_and_vars = opt.compute_gradients(sum(loss_op))
...
def wmethod():
    global word_idx
    return word_idx
evaluate_model(compile_model, ver, qas, pad=1, wmethod=wmethod, flatten=0, repeat=repeat, E2E=1, stop_early=1);
